...
document.observe("dom:loaded", function() {
	 ...
	 */
	 ...
	 for(var i = 0; i < ar.length; i++) {
	 	new Draggable(ar[i], {revert: true});
	 }
	 Droppables.add("selectpad", {onDrop: labSelect});
	 Droppables.add("labs", {onDrop: labDelete});
	});
...
function labSelect(drag, drop, event) {
	 ...
	 */
	 if ($('labs').childElements().indexOf(drag) != -1) {
	 	var cnt = $('selectpad').childElements().length;
	 	if (cnt < 3) {
			 // console.log($(this));
			 $("selection").insert("<li>"+drag.alt+"</li>");
	 ...
	 // dropped.push(new Draggable(drag));
	 // Sortable.create("selection");
	}

	function labDelete(drag, drop, event) {
		if ($('selectpad').childElements().indexOf(drag) != -1) {
			var index = 0;
			for (var i = 0; i < $('selectpad').childElements().length; i++) {
				if (drag === $('selectpad').childElements()[i]) {
					index = i;
					break;
				}
			}
			$('labs').appendChild(drag);
			$$("#selection li")[index].remove();
		}
	}
